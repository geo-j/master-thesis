...
This section will provide the implementation details of the theory part (Section \ref{sec:theory}). The algorithm is implemented in C ++ and makes use of the CGAL library (\url{https://www.cgal.org}). The program takes the segments of a boundary of a simple polygon, and the initial coordinates of the guards as input. For each iteration, the new position of the guards, the area seen by each guard and their movement direction details (momentum, pull) are logged in a file. The position of each guard, their visibility polygon and their movement direction details are plotted using Matplotlib (\url{https://matplotlib.org/}) in Python 3. Lastly, the progress of the total seen area is also plotted.
The project contains 22 code files and 1873 lines of code (excluding comments and empty lines).

Then, given the implementation, we will introduce our experimental setup. The experiments  include basic test polygons to showcase the execution of the algorithm. We also observe the importance of each of the heuristics used. We analyse how the algorithm scales in the context of the comb polygon. Lastly, we will mention the importance of the hyperparameter values used.
