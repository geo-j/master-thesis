...
We know the intersection point between the visibility regions of guards $g_1$ and $g_2$. This point divides the segment seen by $g_1$ behind the reflex vertex $r_1$ into two subsegments of lengths $b_1$ and $b_2$, where $b_1 + b_2 = b$. The length $b_2$ corresponds to the shared seen segment.

As before in equation \ref{eq:ardd}, the area seen by $g_1$ is $$\text{Area}_{\triangle_1 + \triangle_2} = \frac{(b_1 + b_2)^2 \partial y}{2a}.$$
However, we are now only interested in the area that is exclusively seen by $g_1$. Since $\triangle_2$ is also seen by $g_2$, we do not need to take into account when computing the gradient of $g_1$. We thus need to subtract it when computing the area seen by $g_1$. 

The shared seen area is $$\text{Area}_{\triangle_2} = \frac{b_2^2\partial y}{2a}.$$
Thus, the area seen exclusively by $g_1$ can be computed by subtracting the shared seen area from the total area: 
\begin{align}
    \text{Area}_{\triangle_1} &= \text{Area}_{\triangle_1 + \triangle_2} - \text{Area}_{\triangle_2} \\
                              &= \frac{(b_1 + b_2)^2 \partial y}{2a} - \frac{b_2^2\partial y}{2a} \\
    \text{Area}_{\triangle_1} &= \frac{((b_1 + b_2)^2 - b_2^2)\partial y}{2a}. \label{eq:multiple_areas} 
\end{align}

\subsubsection{Computing the Gradient for Multiple Guards}
We can now generalise the gradient computation to multiple guards and intersection points. Let $g$ be the guard whose position we wish to optimise around reflex vertex $r$ as before. Let the areas highlighted in pink in Figure \ref{fig:general_gradient} be the parts of the visibility region of $g$ that are also seen by other guards. Let the visibility region of $g$ intersect other guards' visibility regions in $2n$ intersection points. Let the distance from the reflex vertex $r$ to the intersection points be $b_{11}, b_{12}, ..., b_{n1}, b$.

\begin{figure}[h!]
    \centering
    \includegraphics[width = 0.7\textwidth]{theory/gradient4.png}
    \caption{Canonical gradient construction for where the pink parts of the visible area of $g$ are also seen by other guards. The gray triangles $\triangle_1, ..., \triangle_n$ are the areas that are exclusively seen by $g$.}
    \label{fig:general_gradient}
\end{figure}

We can now generalise equation \ref{eq:multiple_areas}. Namely, we need to subtract the areas that are seen by other guards from the total area seen by guard $g$. We start to move in the direction from the polygon boundary to reflex vertex $r$. We know the intersection points of the shared seen regions with the visibility region of $g$. Thus, we know the length of the segments between the reflex vertex $r$ to the end and to the beginning of the shared visibility regions, respectively. Thus, from the total area seen by $g$ we need to alternatively subtract the area up to the end of the shared visibility region $\triangle_i$, then add back the area to the beginning of $\triangle_i$. This process is repeated for every shared visibility region in order starting from the polygon boundary to the reflex vertex.
...
This results in the fact that the area seen by $g$ exclusively can be computed as $$\text{Area}_{\triangle_1 + \triangle_2 + ... + \triangle_n} = (b^2 - b_{n1}^2 + b_{(n - 1)2}^2 - b_{(n - 1)1}^2 + ... - b_{12}^2 + b_{11}^2)\frac{\partial y}{2a}.$$
Alternatively, if $\triangle_1$ is first seen by other guards (so $b_{11} = 0$), then all the signs are flipped.