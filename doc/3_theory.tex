...
However, we are now only interested in the area that is exclusively seen by $g_1$. Since $\triangle_2$ is also seen by $g_2$, we need to ignore it from the gradient computation of $g_1$. 
...
We can now generalise the gradient computation to multiple guards and intersection points. Let $g$ be the guard whose position we wish to optimise as before. Let the areas highlighted in pink in Figure \ref{fig:general_gradient} be the parts of the visibility region of $g$ that are also seen by other guards. Let there be $n$ intersection points with each of the other guards' visibility regions be known. Hence the distance from the reflex vertex $r$ to them can be computed as $b_1, b_2, ..., b_n$.
...
Generalising equation \ref{eq:multiple_areas}, the area seen by $g$ exclusively can be computed as $$\text{Area}_{\triangle_1 + \triangle_2 + ... + \triangle_n} = (b - b_n + b_{n - 1} - ... + b_1)\frac{\partial y}{2a}.$$
