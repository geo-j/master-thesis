...
Let $P$ be a polygon and $p = (x, y) \in P$ a guard. We are interested in computing the best direction for moving $p$ inside $P$ such that the visibility area $Vis(p)$ increases. That is, exploring what would be a better position $p'$ to move $p$ to such that $p$ ``sees more'' of the polygon $P$. 

We define $f(p) = \text{Area}(p)$ as the area seen by a guard $p$. Let $\bigtriangledown \text{Area}_r(p)$ be the local change in the area guarded by point $p$ around a reflex vertex $r$ seen by $p$ in $P$. Given all reflex vertices $i$, the total (global) change in the area seen by $p$ can be thus summed up to $\text{Area}(p) = \sum_i \text{Area}_i(p)$. Figure \ref{fig:sumf} offers an example for this case for a polygon $P$ and its reflex vertices $r_1$ and $r_2$. The polygon $P$ is guarded by $p$, and its position is modified to $p'$ by a small change $\partial y$ in its $y$-coordinate. The visibility areas of $p$ are $Area_{r_1}$ and $Area_{r_2}$ around reflex vertices $r_1$ and $r_2$, respectively. In this way, the total change in the visibility area of $p$ is computed as $\bigtriangledown \text{Area}(p) = \bigtriangledown \text{Area}_{r_1}(p) + \bigtriangledown \text{Area}_{r_2}(p)$.
...
Let $\bigtriangledown f$ be the gradient of $f$. The gradient then indicates the direction of the steepest descent for the objective function $f(p)$.
...
After the gradient $\bigtriangledown f$ is computed, we can use it to calculate the new optimised position $p'$ of guard $p$: $$p' = p + \alpha\bigtriangledown f.$$
...
to be the gradient of $f$ given that $P$ is guarded by a point $p$. 
...
Take a boundary line segment of $P$, $r$ a reflex vertex inside $P$ and $p$ a guard whose optimal position we are interested in. The reflex vertex $r$ is seen by $p$. Let $\overline{pr} = \alpha$ be known, and let $\triangle rpp' = \triangle_2$. Similarly, let $\beta$ be the known distance between $r$ and the polygon boundary in question.
...
Let $\partial y$ be an extremely small change in the $y$-coordinate of $p$. Let $p'$ be the new position of $p$ given the change $\partial y$. The point $p'$ can see then up to a new point around $r$ on the polygon boundary. Let thus the new observed segment on the polygon boundary be $L$. As such, let $\triangle_1$ denote the increase in the visibility area of $p$ when it moves to position $p'$:
...
We are now interested in computing how the area seen by guard $p$ increases given the change $\partial y$ in the position of $p$. The distances $\alpha$ and $\beta$ are known. As such, we aim at expressing the gradient $\bigtriangledown \text{Area}_r$ for point $p$ and reflex vertex $r \in Vis(p)$ using $\alpha$ and $\beta$. Since $\bigtriangledown \text{Area}_r$ depends on the change in the coordinates of $p$, computing it is tightly connected to the change in the area of triangle $\triangle_1$. We will proceed to calculate the area of $\triangle_1$ below.
...
% \begin{equation}
%     \text{Area}_{\triangle_1} = \frac{\beta^2 \partial y}{2\alpha}. \label{eq:ardd2}
% \end{equation}
...
We can now compute the gradient $\bigtriangledown \text{Area}_r$ in a plane rotated by $R$ for a point $p$ and a reflex guard $r$ seen by $p$ as
...
\begin{align*}
    ...
    % \bigtriangledown \text{Area}_r = &\left(0, \frac{\beta^2}{2\alpha}\right)^\intercal.
\end{align*}

% Analogously, if we rotate the plane such that $p$ and reflex vertex $r$ have the same $y$-coordinate, the gradient becomes $$\bigtriangledown f = (\frac{\beta^2}{2\alpha}, 0)^\intercal.$$

Therefore, for all the reflex vertices $r$ guard $p$ can see, the total gradient $\bigtriangledown f$ becomes the sum of all the partial gradients $\bigtriangledown f_r$ as $$\bigtriangledown f = \sum_{i \in R(p)} \bigtriangledown \text{Area}_i, R(p) = \{\text{reflex vertices of } P \text{ seen by }p\}.$$

\subsubsection{Computing the New Guard's Position}
We can now use the coordinates of the gradient $\bigtriangledown f$ to compute the movement direction of the guard $p$ given all the reflex vertices from $P$ seen by $p$. In order to do so, we will use the construction depicted in Figure \ref{fig:vperp}. 
...
We know that the norm of the gradient is $||\bigtriangledown f|| = \frac{\beta^2}{2\alpha}$. Since $\bigtriangledown f$ has the same direction as $\vec v^\perp$, we wish to normalise it from the norm of $\vec v^\perp$ with $\frac 1 \alpha$. Therefore, the gradient for guard $p$ and one reflex vertex $r \in Vis(p)$ can be computed as $$\bigtriangledown f = \vec v^\perp \frac{\beta^2}{2\alpha} \frac 1 \alpha.$$

As mentioned before, the total gradient for guard $p$ and all the reflex vertices $r$ the guard can see is $$\bigtriangledown f = \sum_{r \in R(p)} \bigtriangledown \text{Area}_r, R(p) = \{\text{reflex vertices of } P \text{ seen by }p\}.$$
