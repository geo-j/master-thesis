...
Let $M_i$ be the momentum for a guard per iteration $i$. In order to compute the momentum, we want to take past gradient descent computations into account with a weight. Let this weight be a hyperparameter $\gamma$. Let $\Delta f_i$ be the computation of the gradient descent in iteration $i$. 
As such, we can take into account the value of the previous gradient $\Delta_{i - 1}$ with weight $\gamma$. However, we still want the gradient $\Delta f_i$ to influence our current movement. This can happen with a weight of $1 - \gamma$. So, the computation for the momentum at iteration $i$ becomes $$M_i = \gamma M_{i - 1} + (1 - \gamma)\Delta f_i.$$

Let $$M_0 = (1 - \gamma) \Delta f_0.$$. We can then observe how  the next iterations of the momentum are carried out:
\begin{align*}
